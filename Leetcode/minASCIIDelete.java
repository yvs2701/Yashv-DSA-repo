/* Leetcode 712: "Minimum ASCII Delete Sum for Two Strings" - based on longest common subseqeunce.
...
class minASCIIDelete {
    ...
    static int minimumDeleteSum(String s1, String s2) {
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                // if both characters are equal then use
                // previous diagonal value which 
                // contains sum of characters which were not equal on last comparison
                if (s1.charAt(i - 1) == s2.charAt(j - 1)) {
                    dp[i][j] = dp[i - 1][j - 1];
                } else {
                    dp[i][j] = Math.min(
                            dp[i - 1][j] + s1.charAt(i - 1), // delete character from s1
                            dp[i][j - 1] + s2.charAt(j - 1) // delete character from s2
                    );
                }
            }
        }
