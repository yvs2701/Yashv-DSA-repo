...
public class BuyAndSellStockII {
    ...
    static int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            if (prices[i] < buy) {
                // if current stock is cheaper than the one already bought
                // then assume you didn't buy that and buy this instead
                buy = prices[i];
            } else {
                profit += prices[i] - buy; // sell whenever you get a profit
                // and immediately buy the stock at current price
                buy = prices[i];
            }
        }
